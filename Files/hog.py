...
def play(strategy0, strategy1, goal=GOAL_SCORE):
    ...
    while (score < goal and opponent_score < goal):
      if who == 0:
        numberOfRolls = strategy0(score, opponent_score)
        score = score + take_turn(numberOfRolls, opponent_score, select_dice(score, opponent_score))
        # Swine swap rule
      ...
      if who == 1:
        numberOfRolls = strategy1(opponent_score, score)
        opponent_score = opponent_score + take_turn(numberOfRolls, score, select_dice(opponent_score, score))
        # Swine swap rule
...
def max_scoring_num_rolls(dice=six_sided):
    ...
    for i in range (1, 11):
      ...
      temp = func(i, dice)
      print (i, "dice scores", temp, "on average")
      averages.append(temp)
      if max(averages) == temp:
        numOfDiceHighestAvgTurnScore = i
    return numOfDiceHighestAvgTurnScore
...
def bacon_strategy(score, opponent_score):
    ...
    0
    """
    return 0 if 1 + max([int(i) for i in str(opponent_score)]) >= BACON_MARGIN else BASELINE_NUM_ROLLS
...
def swap_strategy(score, opponent_score):
    ...
    """
    "*** YOUR CODE HERE ***"
    rolledScore = score + 1 + max([int(i) for i in str(opponent_score)])
    return 0 if rolledScore * 2 == opponent_score else BASELINE_NUM_ROLLS if rolledScore == 2 * opponent_score else bacon_strategy(score, opponent_score)


def final_strategy(score, opponent_score):
    """
    Write a brief description of your final strategy:
    This strategy is one that emphasizes more risk taking if losing and less risk taking if winning. 
    The farther ahead the opponent, the more dice rolled. 
    For example, if the opponent is 20 points ahead, the strategy is to roll 8 die. 
    On the other hand, if the opponent is less than 5 points ahead or is losing, only roll 2 dice. 
    There are conditions for every point in between.
    """
